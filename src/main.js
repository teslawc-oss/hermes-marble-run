...
const BROADCAST_CAMERA = {
  ...
  toyParkBroadcast: {
    ...
    startClose: { back: -5.4, side: 1.25, height: 30.0, lookAhead: 7.8, targetLift: 1.05, fov: 38, positionSmoothing: 0.13, targetSmoothing: 0.18, label: 'start: taller broadcast angle with tighter zoom on the staged front pack' },
    firstCorner: { back: -7.2, side: 2.8, height: 40.0, lookAhead: 12.2, targetLift: 1.25, fov: 36, positionSmoothing: 0.06, targetSmoothing: 0.1, label: 'first corner: taller high-angle chase of the top three with tighter FOV' },
    actionSpot: { back: -6.5, side: -3.0, height: 38.0, lookAhead: 10.8, targetLift: 1.15, fov: 35, positionSmoothing: 0.09, targetSmoothing: 0.12, label: 'mid race: taller high-angle cut to the tightest traffic / overtake pocket with closer zoom' },
    cinematicForward: { back: -10.2, side: 0.35, height: 42.0, lookAhead: 16.0, targetLift: 1.28, fov: 32, positionSmoothing: 0.08, targetSmoothing: 0.11, label: 'intermittent taller cinematic-forward sweep with tighter zoom along the route' },
    leaderDuel: { back: -8.2, side: 2.2, height: 40.5, lookAhead: 13.8, targetLift: 1.25, fov: 33, positionSmoothing: 0.055, targetSmoothing: 0.1, label: 'final: taller high-angle follow on leader plus nearest challenger with tighter zoom' },
    momentZoom: { back: -4.2, side: 1.25, height: 28.0, lookAhead: 7.8, targetLift: 1.05, fov: 30, positionSmoothing: 0.23, targetSmoothing: 0.26, label: 'collision / overtake / lead-change tight zoom punch from a taller angle' },
  },
...
class MarbleRace {
  ...
  getVisualThemeStyles(theme = this.visualTheme) {
    ...
    const patternStyles = (theme.patternKeys || [])
      ...
      .filter(Boolean);
    if (theme.key === 'toyPark' && TOY_PARK_MARBLE_IDENTITIES.length) {
      return {
        colorStyles: TOY_PARK_MARBLE_IDENTITIES.map((identity) => ({
          label: identity.colorLabel,
          hex: identity.hex,
          color: identity.color,
          palette: identity.palette,
          material: identity.materialKey,
        })),
        patternStyles: TOY_PARK_MARBLE_IDENTITIES.map((identity) => ({
          key: identity.patternKey,
          label: identity.patternName,
        })),
  ...
  createMarbleIdentity(index, count = this.marbleData.length) {
    ...
    const toyParkIdentity = theme.key === 'toyPark' && count === TOY_PARK_MARBLE_IDENTITIES.length
      ? TOY_PARK_MARBLE_IDENTITIES[index % TOY_PARK_MARBLE_IDENTITIES.length]
      : null;
    if (toyParkIdentity) {
      const materialKey = toyParkIdentity.materialKey || 'candy';
      const materialStyle = MARBLE_MATERIAL_STYLES[materialKey] || MARBLE_MATERIAL_STYLES.candy;
      const sizeStyle = MARBLE_SIZE_STYLES[index % MARBLE_SIZE_STYLES.length];
      const paletteHex = toyParkIdentity.palette?.length ? toyParkIdentity.palette : [toyParkIdentity.hex];
      const palette = paletteHex.map((hex) => Number.parseInt(hex.replace('#', ''), 16));
      const patternName = toyParkIdentity.patternName || toyParkIdentity.name;
      const visualTagline = toyParkIdentity.tagline || `${materialStyle.label} ${patternName}`;
      const codeNumber = String(index + 1).padStart(Math.max(2, String(count).length), '0');
      const code = `MB-${codeNumber}-TOY-${toyParkIdentity.hex.slice(1, 4).toUpperCase()}-${toyParkIdentity.patternKey.slice(0, 3).toUpperCase()}-${materialKey.slice(0, 3).toUpperCase()}-${sizeStyle.key}`;
      return {
        id: index,
        code,
        name: toyParkIdentity.name,
        displayName: `${code} ${toyParkIdentity.name}`,
        visualThemeKey: theme.key,
        visualThemeName: theme.label,
        visualThemeDescription: theme.description,
        color: toyParkIdentity.color,
        colorHex: toyParkIdentity.hex,
        colorName: toyParkIdentity.colorLabel,
        palette,
        paletteHex,
        materialKey,
        materialName: materialStyle.label,
        patternKey: toyParkIdentity.patternKey,
        patternName,
        visualTagline,
        toyParkCharacterIcon: toyParkIdentity.icon,
  ...
  makeMarbleMesh(radius, color, index, patternKey = 'rings', palette = null, materialKey = 'glass', identity = null) {
    const canvas = document.createElement('canvas');
    canvas.width = 256;
    canvas.height = 128;
    const ctx = canvas.getContext('2d');
    ...
    if (patternKey === 'split') {
      ...
      ctx.fillStyle = accent;
      for (let i = 0; i < 18; i += 1) {
        const x = (i * 41 + 13) % canvas.width;
        const y = (i * 23 + 19) % canvas.height;
        ctx.beginPath();
        for (let p = 0; p < 10; p += 1) {
          const a = -Math.PI / 2 + p * Math.PI / 5;
          const r = p % 2 ? 3 : 8;
          ctx[p ? 'lineTo' : 'moveTo'](x + Math.cos(a) * r, y + Math.sin(a) * r);
        }
  ...
  getDefaultCameraMode() {
    ...
    if (this.state === 'finished') return this.isToyParkViewerOverlayActive()
      ? 'finish'
      : (BROADCAST_CAMERA.podium360.enabled ? 'podium360' : 'finish');
    ...
    if (this.finishers.length > 0) {
      if (this.isToyParkViewerOverlayActive()) return 'finish';
  ...
  updateCamera(delta) {
    ...
    if (activeCameraMode === 'replayHighlight' && this.replayHighlight?.active) {
      ...
      const cfg = this.isToyParkViewerOverlayActive()
        ? (BROADCAST_CAMERA.toyParkFinish || BROADCAST_CAMERA.finish)
    ...
    const baseDesiredFov = activeCameraMode === 'cinematicLeader'
      ...
      : (activeCameraMode === 'toyParkBroadcast'
        ...
        : (activeCameraMode === 'leadPack'
          ...
          : (activeCameraMode === 'finish'
            ? (this.finishCameraState?.fov || BROADCAST_CAMERA.finish.fov || 44)
