...
async function main() {
  ...
  try {
    ...
    const started = await page.evaluate(({ mode, multipleRaceCount, cupSize, trackLength, smokeSeconds, maxRaceSeconds, cupName, ttsVoice, obstaclePreset, obstacleDistribution, obstacleTypes, showLeftUi, showRightUi, disableMouseOrbit, renderPerformanceMode }) => {
      ...
      if (mode === 'single') {
        ...
        app.scheduleContinuousRecordingAction?.(app.continuousRecording.initialGateDelaySeconds, 'waiting-open-gate', () => app.startContinuousRecordingRace?.());
      } else {
